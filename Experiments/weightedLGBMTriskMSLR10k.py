...
#Early stopping is disabled, to use change the early_stopping_rounds param
earlystop = [2000]

for stop in earlystop:
    combineddf = pd.DataFrame()
    name = name + '.earlystopweighted%d' % (stop)
    for fold in range(1,6):
        ...
        baselinename = 'resultsmslr10k/evalMetrics/evalWeighted/baselinevalrun%d' % (fold)
        baselineeval = 'resultsmslr10k/evalMetrics/evalWeighted/baselinevaleval%d' % (fold)
        baselinetrainname = 'resultsmslr10k/evalMetrics/evalWeighted/baselinetrainrun%d' % (fold)
        baselinetraineval = 'resultsmslr10k/evalMetrics/evalWeighted/baselinetraineval%d' % (fold)
        temppath = '/home/sh1/S3713561/work/LightGBM/Experiments/resultsmslr10k/evalMetrics/evalWeighted/'
        ...
        callbacks = set()
        callbacks.add(rru.update_weight())
        ...
        param = {'num_leaves':63, 'num_trees':1500, 'objective':'lambdarank',
             ...
             'verbose': 1, 'early_stopping_rounds': None, 'first_metric_only': True}

        param['metric'] = 'None'
        #Train Model
        num_round = 10
        bst = lgb.train(param, train_data, num_round, valid_sets=[valid_data,train_data], valid_names=['valid','train'], feval=metrics.riskrewardWeighted, callbacks=callbacks, evals_result=eval_result)
    ...
    metrics.evalScore(runPath + 'run.all.' + name, expPath + 'resultsmslr10k/evalPerQueryLGBMTriskweightedes%d' % (stop), qrelsFile)
