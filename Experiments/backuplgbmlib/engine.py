    ...
    for i in range_(init_iteration, init_iteration + num_boost_round):
        for cb in callbacks_before_iter:
            cb(callback.CallbackEnv(model=booster,
                                    ...
                                    risk_reward_list=None))

        booster.update(fobj=fobj)
        
        #Evaluation result list customised for Trisk weighted where the 2nd value returns a list of risk/reward value
        #The final row in that list is the Trisk value                                                               
        evaluation_result_list = []
        risk_reward_list = [] 
        counter = 0           
        # check evaluation result.
        if valid_sets is not None:
            #Change the first evaluation result to valid set
            evaluation_result_list.extend(booster.eval_valid(feval))
            if is_valid_contain_train:
                evaluation_result_list.extend(booster.eval_train(feval))
        try:
            for cb in callbacks_after_iter:
                #Passing riskreward list as a different tuple to callback to calculate                                     
                #Final value of the numpy array is the trisk value, the rest are risk-reward per query                     
                if (counter==0):                                                                                           
                    risk_reward_list = [(eval[0], eval[1], eval[2][0:-1], eval[3]) for eval in evaluation_result_list]     
                    evaluation_result_list = [(eval[0], eval[1], eval[2][-1], eval[3]) for eval in  evaluation_result_list]
                    counter += 1                                                                                           
                cb(callback.CallbackEnv(model=booster,
                                        params=params,
                                        iteration=i,
                                        ...
                                        risk_reward_list=risk_reward_list))
